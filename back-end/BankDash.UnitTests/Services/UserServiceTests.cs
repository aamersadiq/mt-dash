...
namespace BankDash.UnitTests.Services
{
    public class UserServiceTests
    {
        ...
        [Fact]
        public async Task RegisterUserAsync_UserAlreadyExists_ThrowException()
        {
            // Arrange
            var register = new Register { Username = "testuser", Password = "password" };
            var existingUser = new User { Username = "testuser" };
            _roleRepositoryMock.Setup(r => r.GetRoleByNameAsync(RoleType.AccountRead.ToString())).ReturnsAsync(new Role { Id = 1, Name = RoleType.AccountRead.ToString() });
            _userRepositoryMock.Setup(u => u.GetUserByUsernameAsync(register.Username)).ReturnsAsync(existingUser);

            // Act & Assert
            await Assert.ThrowsAsync<Exception>(() => _userService.RegisterUserAsync(register));
        }
    }
}